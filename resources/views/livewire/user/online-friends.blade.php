<div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-4 sticky top-24">
    ...
    <div class="flex items-center justify-between mb-4 px-1">
        ...
        @if(count($friends) > 0)
            <span class="bg-emerald-100 text-emerald-600 text-[10px] font-bold px-2 py-0.5 rounded-full shadow-sm">{{ count($friends) }}</span>
        @endif
    </div>
            ...
            <a href="{{ route('profile', ['id' => $friend->id]) }}" 
               class="group flex items-center gap-3 p-2 rounded-xl hover:bg-slate-50 transition-all duration-200 cursor-pointer border border-transparent hover:border-slate-100">
                ...
                <div class="relative flex-shrink-0">
                    ...
                    <!-- Online Dot (Pulse Effect) -->
                    <span class="absolute bottom-0.5 right-0.5 w-2.5 h-2.5 bg-emerald-500 border-2 border-white rounded-full">
                ...
                <div class="overflow-hidden">
                    <h4 class="font-semibold text-sm text-slate-700 group-hover:text-indigo-600 transition-colors truncate capitalize leading-tight">
        ...
        @if($friends->isEmpty())
            <div class="flex flex-col items-center justify-center py-8 text-center">
                <div class="w-12 h-12 bg-slate-50 rounded-full flex items-center justify-center mb-3">
                    <i data-feather="moon" class="w-5 h-5 text-slate-300"></i>
                </div>
