        ...
        <div class="lg:col-span-9 space-y-6">
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                @if (session()->has('message'))
                    <div
                        class="px-4 py-2 bg-emerald-100 text-emerald-700 text-sm font-bold rounded-lg flex items-center gap-2">
                ...
                <button wire:click="toggleCreate"
                    class="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2.5 rounded-xl font-semibold shadow-lg shadow-indigo-200 transition active:scale-95">
                    @if ($isCreating)
                        <x-heroicon-o-x-mark class="w-4 h-4" /> Close
                    @else
                        <x-heroicon-o-plus class="w-4 h-4" /> New Assignment
                    @endif
                </button>
            </div>
                ...
                <div class="bg-white border border-indigo-100 rounded-2xl p-6 shadow-md mb-6 animate-fade-in-down">
                    ...
                    <form wire:submit.prevent="create" class="space-y-4">
                        ...
                        <div class="flex justify-end pt-2">
                            <button type="submit"
                                class="bg-slate-900 text-white px-6 py-2 rounded-lg font-bold text-sm">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                @forelse($assignments as $assignment)
                    <div wire:click="showDetails({{ $assignment->id }})"
                        ...
                        <div
                            class="absolute left-0 top-0 bottom-0 w-1.5 {{ $assignment->my_submission ? 'bg-emerald-500' : ($assignment->due_date->isPast() ? 'bg-rose-500' : 'bg-indigo-500') }}">
                        </div>

                        <div class="pl-4">
                            <div class="flex justify-between items-start">
                                <span
                                    class="bg-slate-100 text-slate-600 text-[10px] font-bold px-2 py-0.5 rounded uppercase tracking-wider">{{ $assignment->course }}</span>
                                @if ($assignment->my_submission)
                                    <span class="text-emerald-600 text-xs font-bold flex items-center gap-1">
                                        <x-heroicon-o-check class="w-3 h-3" /> Done
                                    </span>
                                @elseif($assignment->due_date->isPast())
                                    <span class="text-rose-600 text-xs font-bold flex items-center gap-1">
                                        <x-heroicon-o-exclamation-circle class="w-3 h-3" /> Overdue
                                    </span>
                                @else
                                    <span class="text-indigo-600 text-xs font-bold flex items-center gap-1">
                                        <x-heroicon-o-clock class="w-3 h-3" /> Pending
                                    </span>
                                @endif
                            ...
                            <h3
                                class="text-lg font-bold text-slate-800 mt-2 group-hover:text-indigo-600 transition-colors line-clamp-1">
                                {{ $assignment->title }}</h3>

                            <div class="flex items-center justify-between mt-4 text-xs font-medium text-slate-500">
                                <span
                                    class="flex items-center gap-1 {{ $assignment->due_date->isPast() && !$assignment->my_submission ? 'text-rose-600' : '' }}">
    ...
    @if ($selectedAssignment)
        <div class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div
                    class="inline-block align-bottom bg-white rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg w-full p-6 relative">

                    <div class="flex justify-between items-start mb-4">
                        <div>
                            <h3 class="text-xl font-bold text-slate-900">{{ $selectedAssignment->title }}</h3>
                            <p class="text-indigo-600 text-xs font-bold uppercase mt-1">
                                {{ $selectedAssignment->course }} • Due
                                {{ $selectedAssignment->due_date->format('F d, Y') }}</p>
                        </div>
                        <button wire:click="closeDetails"
                            class="text-slate-400 hover:text-slate-600 bg-slate-100 p-1 rounded-full">
                    ...
                    <div class="bg-slate-50 p-4 rounded-xl text-sm text-slate-600 mb-6 border border-slate-100">
                        ...
                        @if ($selectedAssignment->file)
                            <div class="mt-3 pt-3 border-t border-slate-200">
                                <a href="{{ asset('storage/' . $selectedAssignment->file) }}" target="_blank"
                                    class="text-indigo-600 text-xs font-bold hover:underline flex items-center gap-1">
                                    <x-heroicon-o-arrow-down-tray class="w-3 h-3" /> Download Attachment
                                </a>
                            </div>
                        @endif
                    </div>
                        ...
                        @if ($selectedAssignment->my_submission)
                            <div class="bg-emerald-50 border border-emerald-100 rounded-xl p-4 text-center">
                                <p class="text-emerald-700 text-sm font-bold flex items-center justify-center gap-2">
                                    <x-heroicon-o-check-circle class="w-4 h-4" /> Submitted Successfully
                                </p>
                                <p class="text-emerald-600 text-xs mt-1">
                                    On
                                    {{ \Carbon\Carbon::parse($selectedAssignment->my_submission->created_at)->format('M d, H:i') }}
                                </p>
                                @if ($selectedAssignment->my_submission->file)
                                    <a href="{{ asset('storage/' . $selectedAssignment->my_submission->file) }}"
                                        target="_blank" class="text-emerald-700 text-xs underline mt-2 block">View
                                        Uploaded File</a>
                                @endif
                            </div>
                        @else
                            <form wire:submit.prevent="submitAssignment" class="space-y-4">
                                <div>
                                    ...
                                    <textarea wire:model="submission_text" rows="3"
                                        class="w-full px-3 py-2 border rounded-lg focus:ring-indigo-500 text-sm"></textarea>
                                </div>
                                ...
                                <div class="flex justify-between items-center pt-2">
                                    @if ($selectedAssignment->user_id === auth()->id())
                                        <button type="button" wire:click="delete({{ $selectedAssignment->id }})"
                                            wire:confirm="Delete task?"
                                            class="text-rose-500 text-xs font-bold uppercase hover:underline">Delete
                                            Task</button>
                                    @else
                                        <div></div>
                                    @endif
                                    ...
                                    <button type="submit"
                                        class="bg-slate-900 text-white px-6 py-2.5 rounded-xl text-sm font-bold hover:bg-slate-800 transition flex items-center gap-2">
