    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div class="lg:col-span-9 space-y-6">
            ...
            @if (session()->has('message'))
                <div class="px-4 py-3 bg-emerald-100 text-emerald-700 rounded-xl flex items-center gap-2 text-sm font-bold">
                    ...
                    <form wire:submit.prevent="create">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                            <div>
                                ...
                                <input type="text" wire:model="title" class="w-full px-3 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none text-sm">
                            ...
                            <div>
                                ...
                                <div wire:loading wire:target="file" class="text-xs text-indigo-600 mt-1 font-bold">Uploading...</div>
                                @error('file') <span class="text-rose-500 text-xs font-bold">{{ $message }}</span> @enderror
                            </div>
                        </div>

                        <div class="flex justify-end">
                            <button type="submit" class="bg-slate-900 text-white px-6 py-2 rounded-lg text-sm font-bold hover:bg-slate-800 flex items-center gap-2">
                    ...
                    @forelse($documents as $doc)
                        ...
                        <div class="p-4 flex items-center gap-4 hover:bg-slate-50 transition">
                            <div class="w-10 h-10 rounded-lg bg-indigo-50 text-indigo-600 flex items-center justify-center flex-shrink-0">
                                <i data-feather="{{ in_array(strtolower($ext), ['pdf', 'doc', 'docx']) ? 'file-text' : 'image' }}" class="w-5 h-5"></i>
                            </div>

                            <div class="flex-1 min-w-0">
                                <h4 class="text-sm font-bold text-slate-800 truncate capitalize">{{ $doc->title }}</h4>
                                <p class="text-xs text-slate-500 mt-0.5 capitalize">
                                    {{ strtoupper($ext) }} • {{ $doc->created_at->format('M d') }} • {{ $doc->user->first_name }}
                                </p>
                            </div>

                            <div class="flex items-center gap-2">
                                <a href="{{ asset('storage/'.$doc->file) }}" download class="p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition">
                                    <i data-feather="download" class="w-4 h-4"></i>
                                </a>
                                
                                @if($doc->user_id === auth()->id())
                                    <button wire:click="delete({{ $doc->id }})" wire:confirm="Delete this file?" class="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition">
