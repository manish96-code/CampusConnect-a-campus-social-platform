<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        @forelse($groups as $group)
            <div class="group bg-white rounded-2xl border border-slate-100 shadow-sm hover:shadow-md hover:border-indigo-100 transition-all duration-300 flex flex-col overflow-hidden relative">
                
                <!-- A. Cover Image Banner -->
                <div class="h-28 w-full bg-slate-100 relative overflow-hidden">
                    @if($group->cover_pic)
                        <img src="{{ asset('storage/' . $group->cover_pic) }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
                    @else
                ...
                <div class="px-5 relative">
                    <div class="absolute -top-8 left-5 w-16 h-16 rounded-2xl border-4 border-white bg-white shadow-md overflow-hidden">
                        @if($group->profile_pic)
                            <img src="{{ asset('storage/' . $group->profile_pic) }}" class="w-full h-full object-cover">
                        @else
                            <div class="w-full h-full bg-slate-50 flex items-center justify-center text-indigo-500 font-bold text-xl uppercase">
